...
- Android **release**（R8）下若出现 `com.sun.jna.Pointer` / `peer field ID` 或导入失败仅 release 复现：检查 `androidApp/proguard-rules.pro` 是否保留 **JNA** 与 **kotlinx.serialization**（见 `docs/reference/SECURITY-ARCHITECTURE.md` 第 7.4 节）
