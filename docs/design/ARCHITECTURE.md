...
|------|------|
| **名称** | SecureVault |
| **定位** | 离线优先的密码管理器；**当前范围：Android + Windows Desktop**（详见 [PLATFORM-SCOPE.md](../PLATFORM-SCOPE.md)） |
| **核心原则** | 零网络依赖、本地加密；活跃目标平台行为一致，暂缓平台不纳入近期验收 |

### 核心功能

1. **密码库管理** — 加密存储密码，支持搜索、分类、收藏
2. **自动填充** — Android AutofillService；Desktop 剪贴板/快捷键（规划）；iOS CredentialProvider **暂缓**
...
│  androidApp / desktopApp（iosApp 暂缓纳入构建与验收）          │
└──────────────────────┬───────────────────────────────────────┘
                       │
┌──────────────────────▼───────────────────────────────────────┐
│                 UI Layer (composeApp)                         │
