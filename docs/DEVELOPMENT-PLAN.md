...
#### Week 1: 项目搭建
- [ ] 初始化 KMP 项目结构（shared/common, shared/android, shared/ios, shared/desktop）
- [ ] 配置 `build.gradle.kts` 和 `libs.versions.toml`
- [ ] 配置 Compose Multiplatform（composeApp 模块）
- [ ] 配置 SQLDelight 跨平台数据库
- [ ] 配置 Koin 依赖注入
- [ ] 搭建 androidApp / iosApp / desktopApp 入口模块

#### Week 2: 加密核心
- [ ] 实现 `AesGcmCipher` (AES-256-GCM 加密/解密)
- [ ] 实现 `SecurePadding` (256 字节块随机填充)
- [ ] 实现 `CryptoUtils` (安全随机、Base64、常量时间比较)
- [ ] 实现 `CryptoConstants` (所有加密常量)
- [ ] 实现 `EncryptedData` 数据格式（v2 存储格式）
- [ ] 编写单元测试：加密/解密往返、填充/反填充、边界条件

#### Week 3: 密钥派生与存储
- [ ] 定义 `Argon2Kdf` expect 接口
- [ ] 实现 Android actual (`argon2kt`)
- [ ] 实现 Desktop actual (`libsodium`)
- [ ] 实现 iOS actual (`libsodium`)
- [ ] 实现 `AdaptiveArgon2Config` 自适应参数
- [ ] 定义 `PlatformKeyStore` expect 接口
- [ ] 实现 Android actual (`AndroidKeyStore`)
- [ ] 编写单元测试：密钥派生参数验证、跨平台一致性

#### Week 4: 安全模块
- [ ] 实现 `SensitiveData<T>` 敏感数据包装器
- [ ] 实现 `MemorySanitizer` 内存安全擦除
- [ ] 实现 `SessionManager` 会话管理
- [ ] 实现 `KeyManager` 密钥管理（设置、解锁、修改密码）
- [ ] 定义 `BiometricAuth` expect 接口
- [ ] 实现 `BiometricState` 防暴力破解
- [ ] 编写集成测试：完整解锁/锁定流程
...
### 验收标准
- [x] iOS 端所有核心功能与 Android 一致
- [x] Face ID / Touch ID 工作正常
- [x] Safari 和第三方 App 自动填充正常
- [x] 安全模式在 iOS 上工作正常
