...
- [x] 已修复“创建账户后冷启动短暂闪回引导页”问题：认证加载完成前不初始化鉴权路由，避免默认 `Onboarding` 初始栈被短暂渲染。
- [x] 已修复“会话超时返回登录页后解锁按钮持续转圈”问题：进入登录路由时重置 `UnlockViewModel` 状态并刷新可用能力，清理残留加载态。
- [x] 已为生物识别自动触发增加稳定性保护：触发前增加短延迟并在触发点二次校验状态；认证流程增加超时兜底，防止异常挂起导致无限 loading。
- [x] 已补充前台门禁：仅在应用处于前台时执行“超时强制回登录”和“登录页自动生物识别”，避免后台误触发生物识别造成回前台卡死。
...
- [x] **待保存草稿落盘**：`AutofillPendingSaveStore`（`commit` 同步写入，**加密 prefs** + 旧明文迁移）+ `MainActivity` 从 Intent 或存储恢复草稿；导航消费后清空。（规范 `openspec/specs/android-autofill/spec.md`。）
